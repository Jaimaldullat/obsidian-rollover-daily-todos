class TodoParser {
  ...
  #withChildren;
  #preserveNonBulletPoints = true;

  constructor(lines, withChildren, preserveNonBulletPoints = true) {
    this.#lines = lines;
 ...
 // Returns a list of strings that represents all the todos along with their potential children
  ...
  getTodos() {
    ...
    for (let l = 0; l < this.#lines.length; l++) {
      ...
      } else if (!this.#isFinishedTodo(line)) {
        // Include all lines that aren't completed todos, including empty lines
        todos.push(line);
      }
    }
    return todos;
...
export const getTodos = ({ lines, withChildren = true, preserveNonBulletPoints = true }) => {
